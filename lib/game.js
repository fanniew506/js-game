...
function play() {
  if (playing === true) {
    ...
    let randomTime = Math.floor(Math.random()* itemInterval + 200);
  ...
  for (let j = 0; j < coinDup.length; j++) {
    if (coinDup[j].y === 700){
      ...
      coinIdx -= 1;
        ...
        if ( coinDup[j].x > position.left - 50 && coinDup[j].x < position.left + 30 && animation.x === position.left ) {
          collectCoin(coinArr[j], j);
        } else if ( coinDup[j].x > position.center - 30 && coinDup[j].x < position.center + 30 && animation.x === position.center ) {
          collectCoin(coinArr[j], j);
        } else if ( coinDup[j].x > position.right - 30 && coinDup[j].x < position.right + 30 && animation.x === position.right ) {
          collectCoin(coinArr[j], j);
        }
    }
  }
