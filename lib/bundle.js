/******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
/******/ 	var installedModules = {};
/******/
/******/ 	// The require function
/******/ 	function __webpack_require__(moduleId) {
/******/
/******/ 		// Check if module is in cache
/******/ 		if(installedModules[moduleId])
/******/ 			return installedModules[moduleId].exports;
/******/
/******/ 		// Create a new module (and put it into the cache)
/******/ 		var module = installedModules[moduleId] = {
/******/ 			exports: {},
/******/ 			id: moduleId,
/******/ 			loaded: false
/******/ 		};
/******/
/******/ 		// Execute the module function
/******/ 		modules[moduleId].call(module.exports, module, module.exports, __webpack_require__);
/******/
/******/ 		// Flag the module as loaded
/******/ 		module.loaded = true;
/******/
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
/******/
/******/
/******/ 	// expose the modules object (__webpack_modules__)
/******/ 	__webpack_require__.m = modules;
/******/
/******/ 	// expose the module cache
/******/ 	__webpack_require__.c = installedModules;
/******/
/******/ 	// __webpack_public_path__
/******/ 	__webpack_require__.p = "";
/******/
/******/ 	// Load entry module and return exports
/******/ 	return __webpack_require__(0);
/******/ })
/************************************************************************/
/******/ ([
/* 0 */
/***/ function(module, exports) {

	"use strict";
	
	Object.defineProperty(exports, "__esModule", {
	  value: true
	});
	var createjs = window.createjs;
	var stage = void 0;
	var animation = void 0;
	var data = {
	  images: ["./images/char.png"],
	  frames: { width: 90, height: 113 },
	  animations: {
	    run: [0, 11, 'run']
	  }
	};
	
	var bombData = {
	  images: ["./images/bomb.png"],
	  frames: { width: 32, height: 32 },
	  animations: {
	    run: [0, 4, 'fizzle']
	  }
	};
	
	var bluCoinData = {
	  images: ["./images/blue_coin.png"],
	  frames: { width: 21, height: 26 },
	  animations: {
	    run: [0, 11, 'sparkle']
	  }
	};
	var tree = void 0;
	var coin = void 0;
	var ratio = ["b", "b", "b", "c", "c", "c", "c", "bc"];
	var numCoins = void 0;
	var position = {
	  center: 290,
	  right: 410,
	  left: 170
	};
	
	var coinIdx = 0;
	var bombIdx = 0;
	var blueCoinIdx = 0;
	var itemSpeed = 4000;
	var itemInterval = 1000;
	
	var gameOverText = void 0;
	var gameOverModal = void 0;
	var bitmap = void 0;
	var bomb = void 0;
	var bombArr = [];
	var coinArr = [];
	var blueCoinArr = [];
	var coinCount = 0;
	var randomPos = [[290, 100], [320, 310], [340, 510]];
	var playing = false;
	var Key = {
	  _pressed: {
	    LEFT: 37,
	    UP: 38,
	    RIGHT: 39,
	    DOWN: 40,
	    ENTER: 13 },
	
	  onKeydown: function onKeydown(event) {
	    if (event.keyCode === this._pressed.RIGHT) {
	      if (animation.x === position.left) {
	        createjs.Tween.get(animation).to({ x: position.center }, 150, createjs.Ease.quadInOut);
	      } else if (animation.x === position.center) {
	        createjs.Tween.get(animation).to({ x: position.right }, 150, createjs.Ease.quadInOut);
	      }
	    } else if (event.keyCode === this._pressed.LEFT) {
	      if (animation.x === position.right) {
	        createjs.Tween.get(animation).to({ x: position.center }, 150, createjs.Ease.quadInOut);
	      } else if (animation.x === position.center) {
	        createjs.Tween.get(animation).to({ x: position.left }, 150, createjs.Ease.quadInOut);
	      }
	    } else if (event.keyCode === this._pressed.ENTER && playing === false) {
	      resetGame();
	    }
	  }
	};
	gameOverModal = new createjs.Shape();
	gameOverModal.graphics.beginFill('#ccc').drawRect(0, 0, 700, 600);
	gameOverModal.alpha = 0;
	gameOverText = new createjs.Text("Game Over\nPress Enter to Play Again", "25px Arial", "#eee");
	gameOverText.x = 130;
	gameOverText.y = 200;
	gameOverText.alpha = 0;
	
	window.addEventListener('keydown', function (event) {
	  Key.onKeydown(event);
	});
	
	function init() {
	  stage = new createjs.Stage("gamecanvas");
	  playing = true;
	  displayRoad();
	  displayCoinCount();
	  addRunningChar();
	  play();
	  addTree();
	  createjs.Ticker.setFPS(30);
	  createjs.Ticker.addEventListener("tick", stage);
	  createjs.Ticker.on("tick", tick);
	}
	
	var addTree = exports.addTree = function addTree() {
	  if (playing === true) {
	    var random = Math.random();
	    tree = new createjs.Bitmap("./images/tree.png");
	    tree.y = 215;
	    tree.scaleX = 0.4;
	    tree.scaleY = 0.4;
	    stage.addChild(tree);
	    if (random < 0.4) {
	      tree.x = 230;
	      createjs.Tween.get(tree).to({ x: -1100, y: 800, scaleX: 9, scaleY: 9 }, 6000, createjs.Ease.quadIn);
	    } else if (random < 0.8) {
	      tree.x = 380;
	      createjs.Tween.get(tree).to({ x: 1050, y: 800, scaleX: 9, scaleY: 9 }, 6000, createjs.Ease.quadIn);
	    } else {
	      tree.x = 500;
	      tree.y = 240;
	      tree.scaleX = 0.2;
	      tree.scaleY = 0.2;
	      createjs.Tween.get(tree).to({ x: 2700, y: 800, scaleX: 9, scaleY: 9 }, 17000, createjs.Ease.quadIn);
	    }
	    setTimeout(addTree, random * 1500 + 200);
	  }
	};
	function addRunningChar() {
	  var spriteSheet = new createjs.SpriteSheet(data);
	  animation = new createjs.Sprite(spriteSheet, "run");
	  animation.y = 410;
	  animation.x = position.center;
	  stage.addChild(animation);
	}
	
	function displayRoad() {
	  var bg = new createjs.Bitmap("./images/background.png");
	  bg.scaleY = 1.18;
	  bg.scaleX = 1.5;
	  bg.x = -300;
	  stage.addChild(bg);
	  var road = new createjs.Bitmap("./images/road.png");
	  road.x = 80;
	  road.y = 250;
	  stage.addChild(road);
	}
	
	function startMenu() {}
	
	function play() {
	  if (playing === true) {
	    var randomIdx = Math.floor(Math.random() * ratio.length);
	    if (ratio[randomIdx] === "b") {
	      giveBomb();
	    } else if (ratio[randomIdx] === "c") {
	      giveCoin("yellow");
	    } else if (ratio[randomIdx] === "bc") {
	      giveCoin("blue");
	    }
	    var randomTime = Math.floor(Math.random() * itemInterval + 200);
	    setTimeout(play, randomTime);
	  }
	}
	
	function giveBomb() {
	  var randomIdx = Math.floor(Math.random() * 3);
	  var bombSheet = new createjs.SpriteSheet(bombData);
	  bomb = new createjs.Sprite(bombSheet, "fizzle");
	  bomb.x = randomPos[randomIdx][0];
	  bomb.y = 240;
	  bomb.scaleX = 0.7;
	  bomb.scaleY = 0.7;
	  bombArr.push(bomb);
	  stage.addChild(bomb);
	  createjs.Tween.get(bomb).to({ x: randomPos[randomIdx][1], y: 600, scaleX: 3.5, scaleY: 3.5 }, itemSpeed, createjs.Ease.quadIn);
	  bombIdx += 1;
	}
	
	function giveCoin(color) {
	  if (color === "yellow") {
	    coin = new createjs.Bitmap("./images/coin.png");
	    coin.name = "yellowCoin";
	  } else if (color === "blue") {
	    var blueCoinSheet = new createjs.SpriteSheet(bluCoinData);
	    coin = new createjs.Sprite(blueCoinSheet, "sparkle");
	    coin.name = "blueCoin";
	  }
	  var randomIdx = Math.floor(Math.random() * 3);
	  coin.x = randomPos[randomIdx][0];
	  coin.y = 240;
	  coin.scaleX = 0.7;
	  coin.scaleY = 0.7;
	  coinArr.push(coin);
	  stage.addChild(coin);
	  createjs.Tween.get(coin).to({ x: randomPos[randomIdx][1], y: 600, scaleX: 3.5, scaleY: 3.5 }, itemSpeed, createjs.Ease.quadIn);
	  coinIdx += 1;
	}
	
	function collectCoin(collectedCoin, idx) {
	  if (collectedCoin.name === "yellowCoin") {
	    coinCount += 1;
	  } else if (collectedCoin.name === "blueCoin") {
	    coinCount += 2;
	  }
	  stage.removeChild(collectedCoin);
	  coinArr.splice(idx, 1);
	  coinIdx -= 1;
	}
	
	function tick() {
	  numCoins.text = "" + coinCount;
	  var coinDup = coinArr;
	  for (var j = 0; j < coinDup.length; j++) {
	    if (coinDup[j].y === 600) {
	      stage.removeChild(coinArr[j]);
	      coinArr.splice(j, 1);
	      coinIdx -= 1;
	    } else if (coinDup[j].y > 380 && coinDup[j].y < 460) {
	      if (coinDup[j].x > position.left - 50 && coinDup[j].x < position.left + 50 && animation.x === position.left) {
	        collectCoin(coinArr[j], j);
	      } else if (coinDup[j].x > position.center - 10 && coinDup[j].x < position.center + 50 && animation.x === position.center) {
	        collectCoin(coinArr[j], j);
	      } else if (coinDup[j].x > position.right - 30 && coinDup[j].x < position.right + 50 && animation.x === position.right) {
	        collectCoin(coinArr[j], j);
	      }
	    }
	  }
	
	  var bombDup = bombArr;
	  for (var _j = 0; _j < bombDup.length; _j++) {
	    if (bombDup[_j].y === 600) {
	      stage.removeChild(bombArr[_j]);
	      bombArr.splice(_j, 1);
	      bombIdx -= 1;
	    } else if (bombDup[_j].y > 400 && bombDup[_j].y < 460) {
	      if (bombDup[_j].x > position.left - 50 && bombDup[_j].x < position.left + 30 && animation.x === position.left) {
	        gameOver();
	      } else if (bombDup[_j].x > position.center - 30 && bombDup[_j].x < position.center + 30 && animation.x === position.center) {
	        gameOver();
	      } else if (bombDup[_j].x > position.right - 30 && bombDup[_j].x < position.right + 30 && animation.x === position.right) {
	        gameOver();
	      }
	    }
	  }
	}
	
	function gameOver() {
	  stage.addChild(gameOverModal, gameOverText);
	  createjs.Ticker.paused = true;
	  playing = false;
	  gameOverText.alpha = 1;
	  gameOverModal.alpha = 0.75;
	}
	
	function resetGame() {
	  createjs.Ticker.paused = false;
	  stage.removeAllChildren();
	  stage.update();
	  coinArr = [];
	  bombArr = [];
	  coinCount = 0;
	  coinIdx = 0;
	  bombIdx = 0;
	  init();
	}
	
	function displayCoinCount() {
	  coin = new createjs.Bitmap("./images/coin.png");
	  coin.x = 600;
	  coin.y = 30;
	  numCoins = new createjs.Text("" + coinCount, "25px Arial", "#f50");
	  numCoins.x = 630;
	  numCoins.y = 50;
	  numCoins.textBaseline = "alphabetic";
	  stage.addChild(numCoins, coin);
	}
	
	document.addEventListener('DOMContentLoaded', function () {
	  init();
	});
	window.coin = coinArr;
	window.stage = stage;

/***/ }
/******/ ]);
//# sourceMappingURL=bundle.js.map